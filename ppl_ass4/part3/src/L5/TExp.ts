...
*/
import { chain, concat, is, map, slice, uniq,sortBy,identity,flatten} from "ramda";
import { Sexp } from "s-expression";
import {typeofDefine } from "./L5-typecheck";
import { isEmpty, isNonEmptyList, second } from "../shared/list";
...
export const makeUnionTExp = (components: TExp[]): UnionTExp => ({tag: "UnionTExp",components:components});
...
export const parseTExp = (texp: Sexp): Result<TExp> =>
    ...
    isArray(texp)&& texp[0] === "union"? parseUnion(texp.slice(1)):
    //isArray(texp)&& texp[0] === "define"? 
...
export const parseUnion = (params: Sexp[]): Result<UnionTExp> =>   
{
    if (params.length != 2)
    {
        return makeFailure("more than 2 arguments");
    } 
         
    let a = params.flat();
    let params2 = a.filter((te)=> te !='union');
    if(params2.length === 6)
    {
        return bind(mapResult(parseTExp,params),(args:TExp[])=>
        makeOk(makeUnionTExp(args)));

    }
    a = a.flat();
    params2 = a.filter((te)=> te !='union');
    params2 = params2.filter((value, index, self) => {return self.indexOf(value) === index;});
    params2.sort();
    return bind(mapResult(parseTExp,params2),(args:TExp[])=>
    makeOk(makeUnionTExp(args)));
}
...
export const unparseTExp = (te: TExp): Result<string> => 
{
    ...
    const unparseUnion = (te: TExp[]): Result<string> => 
    {
        if(te.length ===2)
        {
            te.sort((a, b) => a.tag.localeCompare(b.tag));
            if(te[0].tag === te[1].tag && !isProcTExp(te[0]))
            {
                return bind(unparseTExp(te[0]),(first:string)=>
                makeOk((`(union ${first})`)));
            }
            return bind(unparseTExp(te[0]),(first:string)=>
            bind(unparseTExp(te[1]),(second:string)=>
            makeOk((`(union ${first} ${second})`))));
        }
        

        const tVars = te.filter((te)=> te.tag !="TVar" );
        if(tVars.length === te.length)
        {
            te.sort((a, b) => a.tag.localeCompare(b.tag));
            const te2 = te.filter((item, index, self) => {
                        const foundIndex = self.findIndex(obj => obj.tag === item.tag);
            ...
            return unparseUnion2(te2);
            
        }
        else
        {
            console.log(te);
            return bind(unparseUnion2(te.slice(0,3)),(First:string)=>
            bind(unparseUnion2(te.slice(3)),(second:string)=>
            makeOk((`(union ${first} ${second})`))));
        }
    };

    const unparseUnion2 = (te:TExp[]): Result<string> => 
    {
        if(te.length === 1)
        {
            return bind(unparseTExp(te[0]),(first:string)=>
        ...
        return bind(unparseTExp(te[0]),(first:string)=>
                bind(unparseUnion2(te.slice(1)),(second:string)=>
                makeOk((`(union ${first} ${second})`))));
    };
